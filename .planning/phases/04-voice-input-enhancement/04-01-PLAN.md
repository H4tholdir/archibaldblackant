...
6. Write test cases for **voice recognition error scenarios** (CRITICAL):
   - **Recognition error H71→H61**:
     - Input: "H61.104.032" (spoken H71, heard H61)
     - Expected: `matchType: 'fuzzy'`, suggestions include "H71.104.032" (95% match)
   - **Variant doesn't exist (023→016)**:
     - Input: "845.104.023" (spoken 023, but only .016 exists)
     - Expected: `matchType: 'base_pattern'`, basePattern="845.104", suggestions: [".016", ".032"]
   - **Complete mismatch**:
     - Input: "XXXYYY.999.888" (completely wrong)
     - Expected: `matchType: 'not_found'`, no suggestions
   - **Partial match with variants**:
     - Input: "H129FSQ" (missing last part)
     - Expected: Find all H129FSQ.* variants
...
## Task 6: Add Entity Validation with Fuzzy Matching and Error Recovery

**Objective:** Validate extracted entities against customer/product databases with fuzzy matching and handle voice recognition errors (H71→H61, variant 023→016).

**Steps:**
1. Create enhanced `validateExtractedEntities()` async function with multi-layer validation:
   - Input: `ParsedOrderWithConfidence`, customer list, product list
   - Logic for **customer validation**:
     - Try exact match first
     - If no match, fuzzy match with Levenshtein distance (threshold: 2 edits)
     - Return top 3 suggestions if no exact match
   - Logic for **article validation** (CRITICAL - handles recognition errors):
     - **Layer 1**: Try exact match
     - **Layer 2**: Try base pattern match (first 2 parts)
       - Example: "845.104.023" → find all "845.104.*" variants
       - If base found but variant doesn't exist → return available variants
     - **Layer 3**: Try fuzzy match (handles H71→H61 errors)
       - Use fuse.js with threshold: 0.3 (max 30% difference)
       - Return top 3 similar articles
     - Update confidence scores based on validation results
   - Output: `ValidationResult` with match type and suggestions
2. Install `fuse.js` for fuzzy search:
   ```bash
   cd archibald-web-app/frontend && npm install fuse.js
   ```
3. Implement `ArticleValidationResult` interface:
   ```typescript
...
4. Implement validation logic:
   ...
   ```
5. Add validation error messages with context:
   - **Exact match**: "✓ Articolo H71.104.032 trovato"
   - **Base pattern**: "⚠️ Articolo 845.104 trovato, ma variante .023 non disponibile. Varianti disponibili: .016, .032"
   - **Fuzzy match**: "⚠️ Articolo 'H61.104.032' non trovato. Intendevi: H71.104.032 (95%), H61.104.016 (80%)?"
   - **Not found**: "❌ Articolo non trovato"
