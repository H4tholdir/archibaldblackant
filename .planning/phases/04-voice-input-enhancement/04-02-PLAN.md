...
## Task 6: Create Smart Suggestions Component with Error Recovery

**Objective:** Context-aware suggestions based on what's missing or invalid, with special handling for voice recognition errors.

**Steps:**
1. Enhance `getVoiceSuggestions()` in `orderParser.ts`:
   - Input: transcript, parsed entities, validation results (including `ArticleValidationResult`)
   - Logic:
     ...
     - **If article validation failed**:
       - `matchType: 'base_pattern'` → "⚠️ Variante non trovata. Seleziona: [variant list]"
       - `matchType: 'fuzzy'` → "⚠️ Articolo simile a: [top 3 suggestions with %]"
       - `matchType: 'not_found'` → "❌ Articolo non trovato. Riprova o scrivi manualmente"
     - If quantity missing → "Add 'quantità [number]'"
     - If confidence low → "Speak more clearly: [problem area]"
   - Output: array of actionable suggestions with priority
2. Create enhanced `SmartSuggestions.tsx` component:
   - Props:
     ```typescript
     ...
     ```
   - Rendering based on `matchType`:
     - **exact**: Green checkmark, "✓ Articolo trovato"
     - **base_pattern**: Yellow warning with selectable variant list
     - **fuzzy**: Orange warning with selectable similar articles (with % similarity)
...
3. Create `ArticleSuggestionsList.tsx` sub-component for selectable suggestions:
   ```tsx
   interface ArticleSuggestionsListProps {
     suggestions: Array<{
       code: string;
       variant?: string;
       packageInfo?: string;
   ...
   // Renders:
   // ○ H71.104.032 (95% match) [Select]
   // ○ H61.104.016 (80% match) [Select]
   ```
4. Write unit tests for enhanced suggestion logic
5. Write component tests for all validation scenarios
