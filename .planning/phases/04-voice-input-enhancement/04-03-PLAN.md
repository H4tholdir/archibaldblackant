...
<objective>
Implement voice hybrid workflow: dettatura → form pre-fill → manual edit → tap confirmation. Combines Plans 04-03, 04-04, and 04-05 into single cohesive implementation.
</objective>
...
1. **Dettatura**: User speaks order details (e.g., "H71 104 032 quantità 7")
2. **Package Disambiguation** (if needed): User chooses between 7×1pz or 1×5pz+2×1pz
...
- **Article codes**: Parser handles "H71 104 032" (no "punto") → normalizes to "H71.104.032"
- **Mixed-package scenarios**: When qty=7 with 5-pack+1-pack available:
  - Show disambiguation modal: "📦 3 confezioni (1×5pz + 2×1pz) ✓ Raccomandato" vs "📦 7 confezioni (7×1pz)"
...
2. Add badge to form fields:
   - Customer search input → show badge if `voicePopulatedFields.customer`
   - Article search input → show badge if `voicePopulatedFields.article`
   - Quantity input → show badge if `voicePopulatedFields.quantity`
3. Position badge:
...
1. Add "Review & Apply" button to voice modal:
   ...
   - Shows summary: "✓ Customer: Mario Rossi (95%) | ✓ Article: SF1000 (88%) | ✓ Quantity: 5"
...
3. New workflow:
   - "Add Item" → adds to **draft items list** (not submitted)
   - Draft items shown in review section
   - "Create Order" button → final confirmation modal
...
1. Update `OrderForm.voice.spec.tsx` with new test cases:
   - **Test: Voice input populates form without closing modal**
     - Start voice input
     - Speak "cliente Mario Rossi articolo SF1000 quantità 5"
     - Click "Review & Apply"
   ...
   - **Test: Package disambiguation workflow (CRITICAL)**
     ...
     - Speak "articolo H129FSQ quantità 7"
     - Click "Review & Apply"
     - Assert: Disambiguation modal shown
     - Assert: 2 solutions displayed:
       - "📦 3 confezioni (1×5pz + 2×1pz) ✓ Raccomandato"
       - "📦 7 confezioni (7×1pz)"
   ...
   - **Test: Article code without "punto" (CRITICAL)**
     - Speak "articolo H71 104 032 quantità 5"
   ...
   - **Test: Multi-item voice input**
     - Speak "articolo SF1000 quantità 5, articolo TD1272 quantità 2"
...
1. Add tooltip/hint to voice button:
   - "🎤 Voice Input: Speak your order, then review and confirm"
...
4. **Multi-Item Voice Input:**
   - Say "articolo SF1000 quantità 5, poi TD1272 quantità 2"
   - Assert: Multi-item summary modal shown
   - Select both items
   - Assert: Both items added to draft
