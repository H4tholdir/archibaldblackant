...
export class PDFExportService {
  ...
  generateOrderPDF(order: PendingOrder): jsPDF {
    ...
    try {
      doc.addImage(KOMET_LOGO_BASE64, "PNG", logoX, logoY, logoWidth, logoHeight);
    } catch (error) {
    ...
    doc.text(`Codice: ${order.customerId}`, leftX + 2, infoY + headerHeight + 12);
    ...
    doc.text(`N. preventivo: ${order.id ?? "-"}`, rightX + 2, infoY + headerHeight + 6);
    doc.text(
    ...
    doc.setDrawColor(200, 200, 200);
    doc.setLineWidth(0.3);
    doc.line(summaryX + 2, summaryStartY + 8, summaryX + summaryWidth - 2, summaryStartY + 8);
    ...
    doc.text(`€${orderSubtotal.toFixed(2)}`, summaryX + summaryWidth - 3, currentY, {
      align: "right",
    });
    ...
    doc.text(`€${orderTotal.toFixed(2)}`, summaryX + summaryWidth - 3, currentY, {
      align: "right",
    });
    doc.setTextColor(0, 0, 0); // Reset to black
    ...
    doc.text("I prezzi sono espressi in Euro e sono da intendersi IVA inclusa.", margin, notesY + 15);
    doc.setTextColor(0, 0, 0);
