...
describe.skipIf(!process.env.ARCHIBALD_E2E)('selectDeliveryAddress — E2E diagnostic', () => {
  ...
  it('finds DELIVERYPOSTALADDRESS field after selecting Indelli Enrico (55.227)', async () => {
    ...
    const fieldInfo = await page.evaluate(() => {
      const field = document.querySelector('[id$="DELIVERYPOSTALADDRESS_Edit"]');
      if (!field) return { found: false, fieldId: '', rowCount: 0, rowTexts: [] as string[] };
      const rows = Array.from(document.querySelectorAll('[id*="DELIVERYPOSTALADDRESS_Edit_DDD_gv"] .dxgvDataRow'));
      return {
        found: true,
        fieldId: field.id,
    ...
    console.log('DELIVERYPOSTALADDRESS dump:', JSON.stringify(fieldInfo, null, 2));
    expect(fieldInfo.found).toBe(true);
  }, 120_000);

  it('filters to exactly 1 row when typing Via Francesco Petrarca', async () => {
    const page = (bot as any).page;
    const fieldContainer = await page.$('[id$="DELIVERYPOSTALADDRESS_Edit"]');
