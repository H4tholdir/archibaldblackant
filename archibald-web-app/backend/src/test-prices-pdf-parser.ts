...
async function testPricesPDFParser() {
  ...
  if (prices.length < 4000 || prices.length > 5000) {
    logger.warn(
      `⚠️  Price count unexpected: ${prices.length} (expected ~4,540)`,
    );
  }
  ...
  const pricesWithCurrency = prices.filter(
    (p) => p.unit_price !== null && p.unit_price !== undefined,
  );
  logger.info(
    `Prices with valid unit_price: ${pricesWithCurrency.length}/${prices.length}`,
  );

  if (pricesWithCurrency.length > 0) {
    // Sample prices to show Italian format preserved
    const samplePrices = pricesWithCurrency
      .slice(0, 5)
      .map((p) => p.unit_price);
  ...
  logger.info(
    `Prices with item_selection: ${pricesWithVariants.length}/${prices.length}`,
  );

  // Sample variants
  const variantSamples = pricesWithVariants
    .slice(0, 5)
    .map((p) => p.item_selection);
  ...
  logger.info(
    "  Page 3: IMPORTO UNITARIO (key field), VALUTA, UNITÀ DI PREZZO",
  );
