...
export class ArchibaldBot {
    ...
    try {
      ...
      await this.runOp(
        ...
        async () => {
          ...
          logger.debug(
            'Cerco dropdown a destra del campo "Account esterno"...',
          );
          ...
          if (!dropdownClicked) {
            const fallbackHandle = await this.page!.evaluateHandle(
              (inputId: string) => {
                ...
                const following = Array.from(
                  ...
                  (el) => el.id.includes("_B-1") || el.id.includes("_DDD"),
                );
                return following[0] || null;
              },
              customerInputId,
            );
            if (fallbackHandle && "asElement" in fallbackHandle) {
              const element =
                fallbackHandle.asElement() as ElementHandle<Element> | null;
          ...
          if (rows.length > 0) {
            ...
            const firstCell = await rows[0].$("td");
            const clickTarget: ElementHandle<Element> = firstCell || rows[0];
      ...
      for (let i = 0; i < orderData.items.length; i++) {
        ...
        await this.runOp(
          ...
          async () => {
            ...
            logger.debug(
              "Pulsante New cliccato, attendo apertura nuova riga...",
            );
        ...
        await this.runOp(
          `order.item.${i}.article.find_input`,
          async () => {
            logger.debug(
        ...
        let popupContainer: ElementHandle<Element> | null = null;
        let searchInput: ElementHandle<Element> | null = null;
        let selectedVariant: ReturnType<
          typeof this.productDb.selectPackageVariant
        > = null;

        await this.runOp(
          `order.item.${i}.article.open_dropdown`,
          async () => {
            const dropdownSelectors = [
            ...
            const isArticlePopupOpen = async (): Promise<boolean> => {
              ...
              if (genericPopup && (await genericPopup.boundingBox()))
                return true;
        ...
        await this.runOp(
          `order.item.${i}.article.find_search`,
          async () => {
            ...
            if (!searchInput) {
              ...
              for (const candidate of candidates) {
                const info = await candidate.evaluate((el: Element) => {
        ...
        await this.runOp(
          `order.item.${i}.article.search_type`,
          async () => {
            if (!searchInput) {
              throw new Error("Barra ricerca articolo non trovata");
            }
            ...
            selectedVariant = this.productDb.selectPackageVariant(
              item.articleCode,
              item.quantity,
            );
        ...
        await this.runOp(
          `order.item.${i}.article.select_row`,
          async () => {
            if (!selectedVariant) {
              throw new Error("selectedVariant is not set");
            }
        ...
        await this.runOp(
          `order.item.${i}.wait_loading_complete`,
          async () => {
            logger.debug(
        ...
        let quantityInputId = "";
        let quantityBaseId = "";
        let quantityInput: ElementHandle<Element> | null = null;

        await this.runOp(
          `order.item.${i}.quantity.find_input`,
          async () => {
        ...
        await this.runOp(
          `order.item.${i}.quantity.activate_cell`,
          async () => {
            ...
            if (!clicked) {
              ...
              if (!quantityInput) {
                throw new Error(
                  "Campo quantita articolo non trovato dopo retry",
                );
              }
        ...
        await this.runOp(
          ...
          async () => {
            ...
            if (!Number.isNaN(parsedValue) && parsedValue !== quantityValue) {
              await quantityInput!.evaluate((el: Element, value: string) => {
                const input = el as HTMLInputElement;
                input.value = value;
                input.dispatchEvent(new Event("input", { bubbles: true }));
                input.dispatchEvent(new Event("change", { bubbles: true }));
        ...
        if (item.discount && item.discount > 0) {
          ...
          await this.runOp(
            `order.item.${i}.discount.find_input`,
            async () => {
      ...
      const orderId = await this.runOp(
        "order.save_and_close",
        async () => {
          logger.info(
            '🤖 BOT: Click su "Salva e chiudi" per salvare l\'ordine',
          );
  ...
  async downloadCustomersPDF(context: BrowserContext): Promise<string> {
    ...
    try {
      ...
      if (error.message?.includes("timeout")) {
        throw new Error(
          "PDF download timeout (120s exceeded). Archibald may be generating PDF or button selector changed.",
        );
      }
  ...
  async downloadProductsPDF(context: BrowserContext): Promise<string> {
    ...
    try {
      ...
      logger.info("[ArchibaldBot] Navigated to Products ListView page");

      // Wait for dynamic content to load (same pattern as customers sync)
      await new Promise((resolve) => setTimeout(resolve, 2000));
      ...
      const timestamp = Date.now();
      const userId = this.userId || "unknown";
      const downloadPath = `/tmp/prodotti-${timestamp}-${userId}.pdf`;
      ...
      const downloadComplete = new Promise<void>((resolve, reject) => {
        ...
        // Poll for file creation
        const checkFile = setInterval(() => {
          // Look for Prodotti.pdf/Products.pdf (Archibald's default names) or our custom pattern
          const files = fs.readdirSync("/tmp");
          const pdfFiles = files.filter(
            (f: string) =>
              f === "Prodotti.pdf" ||
              f === "Products.pdf" ||
              (f.startsWith("prodotti-") && f.endsWith(".pdf")) ||
              (f.startsWith("products-") && f.endsWith(".pdf")),
          );

          if (pdfFiles.length > 0) {
            // Find the most recent one (prefer Prodotti.pdf/Products.pdf if they exist)
            const recentPdf =
              pdfFiles.find((f: string) => f === "Prodotti.pdf") ||
              pdfFiles.find((f: string) => f === "Products.pdf") ||
              pdfFiles[pdfFiles.length - 1];
            const tempPath = `/tmp/${recentPdf}`;
