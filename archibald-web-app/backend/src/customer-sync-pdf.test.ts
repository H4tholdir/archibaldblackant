...
skipInCI("CustomerSyncService PDF-based integration", () => {
  ...
  it("should handle background sync with retry logic", async () => {
    // Call runBackgroundSync directly to test retry mechanism
    const runBackgroundSync = (service as any).runBackgroundSync.bind(service);
  ...
  it("should validate sync duration within performance target", async () => {
