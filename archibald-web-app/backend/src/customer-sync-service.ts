...
export class CustomerSyncService extends EventEmitter {
  ...
  async syncCustomers(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const dropdownsInfo = await bot.page!.evaluate(() => {
          ...
          const relevantTds = allTds
            .filter(td => {
              ...
              return text.includes('clienti') || text.includes('liberi') || text.includes('esclusi');
            })
            .slice(0, 10);
        ...
        const filterDropdownSelected = await bot.page!.evaluate(() => {
          ...
          for (const input of allInputs) {
            ...
            if (id.includes('ITCNT8')) {
              ...
              if (value.toLowerCase() === 'tutti i clienti' && hiddenField && hiddenField.value === 'xaf_xaf_a0All_Customers') {
        ...
        if (filterDropdownSelected.found) {
          if ((filterDropdownSelected as any).needsSelection) {
            logger.info(`Filtro trovato con valore input="${(filterDropdownSelected as any).currentValue}", hidden="${(filterDropdownSelected as any).hiddenValue}" - imposto "Tutti i clienti"...`);
            ...
            try {
              ...
              if (result.success) {
                logger.info(`✓ Filtro "Tutti i clienti" impostato via campo nascosto`, {
          ...
          logger.warn('⚠ Impossibile trovare il dropdown filtro clienti, procedo comunque...');
        }
      } catch (error) {
      ...
      const allCustomers: Array<{ id: string; name: string; vatNumber?: string; email?: string }> = [];
      ...
      while (hasMorePages && currentPage <= 100 && !this.shouldStop) {
        ...
        const pageCustomers = await bot.page!.evaluate(() => {
          const rows = Array.from(document.querySelectorAll('table tbody tr')) as Element[];
          const results: Array<{ id: string; name: string; vatNumber?: string; email?: string }> = [];
          ...
          for (const row of rows) {
            ...
            const accountNumber = (cells[2] as Element)?.textContent?.trim() || '';
            const customerCode = (cells[3] as Element)?.textContent?.trim() || '';
            const companyName = (cells[4] as Element)?.textContent?.trim() || '';
            ...
            if (!accountNumber ||
                ...
                !/\d/.test(accountNumber)) {  // accountNumber deve contenere almeno una cifra
              continue;
            }
        ...
        logger.info(`Estratti ${pageCustomers.length} clienti dalla pagina ${currentPage}`);
        ...
        hasMorePages = await bot.page!.evaluate(() => {
          ...
          for (const btn of nextButtons) {
            if (btn && !(btn as HTMLElement).classList?.contains('dxp-disabled') &&
                !(btn.parentElement as HTMLElement)?.classList?.contains('dxp-disabled')) {
              return true;
            }
          }
        ...
        if (hasMorePages) {
          ...
          if (!clicked) {
            ...
            logger.info(`🔄 Re-impostazione filtro "Tutti i clienti" dopo paginazione a pagina ${currentPage + 1}...`);
            await ensureAllCustomersFilter();
      ...
      logger.info(`Estrazione completata: ${allCustomers.length} clienti da ${currentPage} pagine`);
