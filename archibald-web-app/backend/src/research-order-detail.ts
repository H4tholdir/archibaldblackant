...
async function researchOrderDetail() {
  ...
  try {
    ...
    const orderNumber = await orderLink.evaluate((el) =>
      el.textContent?.trim(),
    );
    ...
    const pageAnalysis = await page.evaluate(() => {
      ...
      document
        .querySelectorAll("label, .label, .field-label, td.label, span.label")
        .forEach((label) => {
      ...
      const invoiceMatches =
        bodyText.match(/fattura|invoice|CFT\/\d+|CF1\/\d+/gi) || [];
      const ddtMatches =
        bodyText.match(/DDT\/\d+|documento.*trasporto/gi) || [];
    ...
    if (
      pageAnalysis.containsInvoiceReferences &&
      pageAnalysis.containsDDTReferences
    ) {
      console.log(
        "✅ EXCELLENT: Both invoice AND DDT references found on order detail page!",
      );
      console.log(
        "   → Can use this page to link orders ↔ invoices ↔ DDTs directly",
      );
      ...
      console.log(
        "⚠️  DDT references not found - will need to match DDT by order number",
      );
      ...
      console.log(
        "⚠️  Invoice references not found - will need complex matching strategy",
      );
    } else {
      console.log("⚠️  No direct invoice or DDT references found");
      console.log(
        "   → Will need to match by customer ID + date range + amount",
      );
    }
